...
def test(x):
    print(x)
def get_data(Stock_ticker, start="2020-01-01", end="2025-08-22"):
    #input date + 1
...
def get_list_of_changes_prop_to_mean_at_a_price(data, quote):
    ...
    for x in range(len(stonk_data) - 1):
        close, close_next_day = (float(stonk_data.iloc[x,3]), float(stonk_data.iloc[x + 1,3]))
...
def get_list_of_changes_prop_to_mean_over_time_period_at_a_price(data, time_period, price):
    ...
    for x in range(len(stonk_data) - time_period):
        close, close_next_day = (float(stonk_data.iloc[x,3]), float(stonk_data.iloc[x + time_period,3]))
